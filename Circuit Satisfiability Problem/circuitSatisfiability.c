...
int main (int argc, char *argv[]) {
   ...
   int id = -1, numProcesses = -1; // process id, -1 omdat het ook in opg 5.2 zo was
   int count = 0, totaal = 0;        // number of solutions 
   ...
   for (i = 0; i <= UINT_MAX; i+= numProcesses) {
   ...
   // mpitutorial.com/tutorials/mpi-reduce-and-allreduce/
   MPI_Reduce(&count, &totaal, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);
   // MPI_Allreduce(&count, &totaal, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
